...
const char* ssid = "INFINITUM9285_2.4";  // Aquí debes poner el nombre de tu red, en nuestro caso generamos una red local
const char* password = "T97PPtRFYC";  // Aquí debes poner la contraseña de tu red
//Datos del broker MQTT
const char* mqtt_server = "192.168.1.178"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    ilum1=((long)V1*A1*10)/((long)Ba*Rc1*(1024-V1));
    Serial.println(ilum1);
    if(ilum1>5)
    msj1="Cajon Libre";
    else
    msj1="Cajon Ocupado";*/
    /*valor_ilum1=analogRead(LDR1);
    //valor_ilum1=20;    
    Serial.println(valor_ilum1);
    delay(1000);*/
    //LeerLDR();
  ...
  delay(1000); 
    ...
    /*if (isnan(valor)) {
      Serial.println(F("Failed to read from LDR 1 sensor!"));
      return;
    }*/

    //printLocalTime();
    String json = "{\"id\":\"Estado:\",\"sensor1\":"+String(valor)+"}";
    //String json = "{\"id\":\"Estado:\",\"sensor1\":"+String(valor)+",\"sensor2\":"+String(valor2)+"}";
    ...
    client.publish("FCAeI/mqtt/GMA/LDR", char_array); 
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "FCAeI/mqtt/GMA/LDR") {  // En caso de recibirse mensaje en el tema codigoIoT/ejemplo/mqttin
    if(messageTemp == "true"){
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("FCAeI/mqtt/GMA/LDR"); // Esta función realiza la suscripción al tema
